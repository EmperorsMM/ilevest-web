...
-- Apply AFTER Stages 1-3. Revised per rulings: NO stored prices (Ops invoices per request).
-- Adds the buyer status projection and the buyer-document store (allow-zero + encourage,
-- per-service checklist), all RLS/ownership-scoped. Idempotent: if an earlier priced version
-- was applied, this safely drops the price columns and the quote. One transaction; re-runnable.
-- =============================================================================
begin;
