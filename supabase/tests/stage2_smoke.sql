...
do $$ declare nlines int; begin
  ...
  if nlines <> 7 then raise exception 'FAIL: complete bundle should add 7 lines, got %', nlines; end if;
  raise notice 'PASS: complete bundle expanded to 7 order lines';
end $$;
...
do $$ declare res jsonb := current_setting('t.r1')::jsonb; nc int; begin
  if (res->>'checks_created')::int <> 7 then raise exception 'FAIL: fan-out should create 7 checks, got %', res->>'checks_created'; end if;
  ...
  if nc <> 7 then raise exception 'FAIL: order1 should have 7 checks, got %', nc; end if;
  raise notice 'PASS: confirm_payment created 7 checks from the bundle';
end $$;
...
do $$ declare res jsonb := current_setting('t.r2')::jsonb; nc int; begin
  ...
  select count(*) into nc from public.check_item where order_id='cc000000-0000-0000-0000-000000000001';
  if nc <> 7 then raise exception 'FAIL: still expect 7 checks after duplicate webhook, got %', nc; end if;
  raise notice 'PASS: duplicate webhook is idempotent (no double fan-out)';
end $$;

\echo ''
\echo '################ PART 1b — diaspora bundle includes Identity Verification (C1-PE-02) ################'
set role authenticated; set app.user_id = :ops;
...
do $$ declare res jsonb := current_setting('t.rd')::jsonb; nk int; begin
  if (res->>'checks_created')::int <> 9 then raise exception 'FAIL: diaspora should create 9 checks, got %', res->>'checks_created'; end if;
  select count(*) into nk from public.check_item where order_id='cc000000-0000-0000-0000-000000000003' and service_code='C1-PE-02';
  if nk <> 1 then raise exception 'FAIL: diaspora should include the Identity Verification check (C1-PE-02)'; end if;
  raise notice 'PASS: diaspora expands to 9 checks including Identity Verification (C1-PE-02)';
end $$;
